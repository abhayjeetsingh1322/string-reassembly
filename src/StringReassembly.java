...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //Creating a string variable to hold the concatenated string
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //Declaring a temporary set
        Set<String> temp = strSet.newInstance();

        //Declaring a boolean variable for substring condition
        boolean isSubstring = false;

        //Loop to go through set and also checking for the substring condition
        while (!(strSet.size() == 0) && !(isSubstring)) {

            //Getting the a set string from set
            String currentStr = strSet.removeAny();

            //Checking if the set string contains the substring
            if (currentStr.contains(str)) {
                isSubstring = true;
            }

            //Adding to temporary set
            temp.add(currentStr);
        }

        //If condition if string is not a substring
        if (!isSubstring) {

            //Loop through temporary set as it contains all the original values
            while (!(temp.size() == 0)) {

                //Getting the a set string from the set
                String currentStr = temp.removeAny();

                //Checking if the set string is a substring of the string passed
                if (!str.contains(currentStr)) {
                    strSet.add(currentStr);
                }
            }
            ...
            //Restoring strSet by adding the elements removed in temporary set
        } else {
            strSet.add(temp);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> setOfStrs = new Set3<>();

        //Loop checking if end of file is reached
        while (!input.atEOS()) {

            //Removing the line
            String currentStr = input.nextLine();

            //Calling addToSetAvoidingSubstrings on set of strings and the line
            addToSetAvoidingSubstrings(setOfStrs, currentStr);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        while (index < text.length()) {
            ...
            //Checking if the character is ~ and outputting accordingly
            if (text.charAt(index) == '~') {
                out.println();
            } else {
                out.print(text.charAt(index));
            }
            index++;
        }
